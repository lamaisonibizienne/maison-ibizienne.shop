...
import { ShoppingBag, X, Instagram, Facebook, Loader, ChevronRight, Menu, ArrowRight, Heart } from 'lucide-react';
...
// ==============================================================================
// 2. ZONE DE TEXTE MODIFIABLE (C'est ici que vous changez vos textes !)
// ==============================================================================
  ...
  <div className="relative h-[95vh] w-full flex flex-col justify-center items-center text-center px-4 overflow-hidden bg-[#F2F0EB]">
    ...
    <div className="relative z-10 pt-40 max-w-4xl animate-slide-up">
      <div className="bg-white/10 backdrop-blur-sm border border-white/20 p-8 md:p-14 inline-block shadow-2xl">
        <span className="text-[10px] uppercase tracking-[0.4em] text-white/90 mb-6 block font-serif">
          {TEXTS.hero.surtitle}
        </span>
        <h1 className="text-4xl md:text-6xl lg:text-7xl font-serif text-white mb-8 leading-[0.85] tracking-tight drop-shadow-lg whitespace-pre-line">
          {TEXTS.hero.title}
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {validCollections.map((col) => (
          <div 
            ...
            className="group cursor-pointer relative h-[500px] overflow-hidden bg-[#F0EBE5]"
          >
            <img 
              src={col.node.image?.url || "https://via.placeholder.com/800"} 
              alt={col.node.title} 
          ...
          return (
            <div key={node.id} className="group cursor-pointer">
              <div className="relative aspect-[3/4] bg-[#F5F2EB] mb-6 overflow-hidden">
                ...
                <button 
                  onClick={() => onAdd(node)}
                  className="absolute bottom-4 right-4 bg-[#FDFBF7] p-3 rounded-full shadow-lg opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 hover:bg-stone-900 hover:text-white"
                >
    ...
    <section id="journal" className="py-32 bg-[#F5F2EB]">
      <div className="max-w-[1600px] mx-auto px-6 md:px-12">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-6 border-b border-stone-300 pb-8">
          ...
          <a href="#" className="text-xs uppercase tracking-widest text-stone-600 hover:text-stone-900 transition-colors font-serif">{TEXTS.sections.journal_link}</a>
        </div>
          ...
          {articles.map((article, idx) => {
            ...
            return (
              <div key={idx} className="group cursor-pointer">
                ...
                <div className="pr-6">
                  <span className="text-[10px] uppercase tracking-[0.2em] text-stone-500 block mb-3 font-serif">
  ...
  <footer className="bg-[#1C1C1C] text-[#FDFBF7] py-24">
    <div className="max-w-[1800px] mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-4 gap-12">
      <div className="md:col-span-1">
        ...
        <p className="text-stone-400 text-sm font-serif italic leading-relaxed max-w-xs">
          {TEXTS.footer.about}
        </p>
      </div>
      <div>
